...
spectronautPeptideFile <- "/Users/ben/Box/5HT2A data analysis/Phosphoproteomics/Spectronaut results_local 0.75/LSD_Phospho Report.xls"
...
contrastPatterns <- c("LSD.*-LSD_0", "LSD.*-ACN_60")
...
# choose either PH or UB to match the data
site.PH_or_UB <- "PH"
...
cf<- list()
# normalization method FALSE = no normalization; default is global medians which you can get my removing/commenting out the line
...
observationCounts <- normPepInt[!is.na(INTENSITY) & INTENSITY > minTrustedIntensity,.N, by = PEPTIDE]
...
ggplot (normPepInt[PEPTIDE %in% completePeptideIons], aes(x=interaction(GROUP_ORIGINAL, SUBJECT_ORIGINAL), y = ABUNDANCE)) +
  ...
  labs(title = sprintf("%d peptide ions detected in at least %d runs with intensity > %d", length(completePeptideIons), numRunsRequired, minTrustedIntensity))
...
kinaseSummaryScores.csv <- file.path(paste0(outFilePrefix, "_output", collapse=""), "kinaseSummaryScores.csv")
kinaseSubstrateData.csv <- file.path(paste0(outFilePrefix, "_output", collapse=""), "kinaseSubstrateData.csv")
